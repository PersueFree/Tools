...
const AppConfusionConversion: FC = () => {
  const dispatch = useAppDispatch();
  const poc = useAppSelector((state) => state.app.poc);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      dispatch(setPoc_state({ ...formValue, result }));
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <FormEvent form={form} autoComplete='off' initialValues={{ type: 0 }}>
        <ButtonContent>
          <Form.Item name='confuseKey' rules={[{ required: true, message: "请输入混淆key" }]}>
            <InputEvent placeholder='例如：ph_vamo ph_cash' />
          </Form.Item>
          <Form.Item name='type'>
            <SelectEvent
              options={[
...
export default AppConfusionConversion;
